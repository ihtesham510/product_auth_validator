...
export function PrizeDefinitions() {
	...
	const [editingPrize, setEditingPrize] =
		useState<Id<'prize_definitions'> | null>(null)
	...
	const handleDeletePrize = async (prizeId: Id<'prize_definitions'>) => {
		if (confirm('Are you sure you want to delete this prize definition?')) {
			try {
				await deletePrizeDefinition({ prize_definition_id: prizeId })
			} catch (err) {
